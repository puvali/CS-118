...
public:
  ...
  // Added in; not part of skeleton code
  void 
  incoming_arp_request(arp_hdr * arp_h, const uint8_t * buf, const size_t size, const Interface* iface);

  void 
  handlePing(ip_hdr * ip_h,  icmp_hdr * icmp_h, const uint8_t * buf, const size_t size, const Interface* iface);

  void
  handleTracerouteToRouter(ip_hdr * ip_h, const uint8_t * buf, const Interface* iface);

  void
  handleZeroTTL(ip_hdr * ip_h, const uint8_t * buf, const Interface* iface);
