...
#include <string.h>
#include <unistd.h>
#include <iostream> //added by Nandini
...
RoutingTable::lookup(uint32_t ip) const
{
    ...
    const RoutingTableEntry *entry_pointer = NULL;
    std::list<RoutingTableEntry>::const_iterator it;

    //check each entry in the routing table
    for (it = m_entries.begin(); it != m_entries.end(); it++)
      {
        uint32_t masked_ip = ip & it->mask;
        uint32_t masked_dest = it->dest & it->mask;

        //entry with matching prefix found
        if (masked_ip == masked_dest)
          {
            //longest prefix match?
            if (masked_ip >= longest_match)
              {
                longest_match = masked_ip;
                entry_pointer = &(*it);
              }
          }
      }
    ...
    if(!entry_pointer)
      throw std::runtime_error("Routing table entry not found\n");
